function [circuit, subnormalization_factor, info] = siable( A, compr_type, compr_val, logging, offset )
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    global_phase = 0;
    [ circuit, phase, info_subcircuit ] = ucrz(circuit, U(:,end), logging); 
    if logging
    ...
    for i = pow2(NumQubits)-1:-1:1
        [ circuit, info_subcircuit ] = double_ucry( circuit, Theta(i,:), Theta_index(i), logging ) ; 
        if logging
            nRY = nRY + info_subcircuit.nG ;
            nCNOT = nCNOT + + info_subcircuit.nCNOT ; 
        end
        [ circuit, phase, info_subcircuit ] = ucrz( circuit, U(:,i), logging ) ;
        if logging
            nRZ = nRZ + info_subcircuit.nRZ ;
            nCNOT = nCNOT + + info_subcircuit.nCNOT ;
        end
...
function [ circuit, info ] = UniformRotation( circuit, ctrl_type, para_seq, ctrl_index, targ_index, logging )
...
function [circuit, global_phase, info ] = ucrz(circuit, theta, logging)
...
% output: exp(-0.5i.*global_phase).*diag(circuit.matrix).' = exp(theta.*1i)
    ...
    for i = 2:n
        ...
        [circuit, info]  = UniformRotation(circuit,'RZ',theta(pow2(i-1)+1:pow2(i))',0:i-2,i-1,logging) ;
        if logging 
...
function [circuit, info ] = double_ucry(circuit, theta, theta_index, logging)
...
% -------------------------------------------------------------------------
    ...
    [circuit, info ] = UniformRotation( circuit,'RY',theta,ctrl_index,theta_index-1, logging );
