function [circuit, subnormalization_factor, info] = bitble( A, compr_type, compr_val, logging, offset )
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % circuit
    [ circuit, info_circ ] = GenerateSingleControlQCircuit( Varphi, HatTheta, offset, logging );
    if logging, info.circ = info_circ.circ; end
end


function [ circuit, info ] = GenerateSingleControlQCircuit(Varphi, Theta, offset, logging)
...
%         info     -- struct containing some info on compression algorithm and circuit
    ...
    if iscomplex
        [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RZ', Theta(pow2(n),:), n:(2*n-1), 0, logging ) ; 
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
    end
    [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RY', Varphi(1,:), n:(2*n-1), 0, logging ) ; 
    if logging
    ...
    for k = 1:n-1
        ...
        [circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RY', Varphi(para_row_index,:)', 0:n-1, k, logging ) ; 
        if logging
            nRY = nRY + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
    ...
    if iscomplex
        [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RZ', Theta(1,:), n:(2*n-1), 0, logging) ; 
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
        for k = 1:n-1
            para_row_index = pow2(k):pow2(k+1)-1;
            [circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RZ', Theta(para_row_index,:)', 0:n-1, k, logging ) ; 
            if logging
                nRZ = nRZ + info_subcircuit.nG ;
                nCNOT = nCNOT + info_subcircuit.nCNOT ; 
            end
    ...
    for k = 1:n-1
        para_row_index = pow2(k):pow2(k+1)-1;
        [circuit_GLOBAL, info_subcircuit ] = UniformRotation( circuit_GLOBAL, 'RY', Varphi_global(para_row_index,:)', 0:n-1, k, logging ) ;
        if logging 
            nRY = nRY + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ;
        end
    ...
    while i <= pow2(n)
        if any(para_seq(:,i) ~= 0)
            ...
            if size(para_seq, 1) == 1
                ...
                [ circuit, info_subcircuit, parity_check ] = UniformRotation( circuit, ctrl_type, para_seq(:,i)', length(ctrl_index):(2*length(ctrl_index)-1), targ_index, logging );
                if logging 
