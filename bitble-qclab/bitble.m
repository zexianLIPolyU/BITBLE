...
%
% Reference: Binary Tree Block Encoding of Classical Matrix.
...
%         info     -- struct containing some info on compression algorithm and circuit
    ...
    if iscomplex
        [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RZ', Theta(pow2(n),:), int32(n:(2*n-1)), int32(0), logging, circuit_sim ) ; 
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
    end
    [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RY', Varphi(1,:), int32(n:(2*n-1)), int32(0), logging, circuit_sim ) ; 
    if logging
    ...
    for k = 1:n-1
        ...
        [circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RY', Varphi(para_row_index,:)', int32(0:n-1), int32(k), logging, circuit_sim ) ; 
        if logging
            nRY = nRY + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
    ...
    if iscomplex
        [circuit_PREP, info_subcircuit] = UniformRotation( circuit_PREP, 'RZ', Theta(1,:), int32(n:(2*n-1)), int32(0), logging, circuit_sim ) ; 
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
        for k = 1:n-1
            para_row_index = pow2(k):pow2(k+1)-1;
            [circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RZ', Theta(para_row_index,:)', int32(0:n-1), int32(k), logging, circuit_sim ) ; 
            if logging
                nRZ = nRZ + info_subcircuit.nG ;
                nCNOT = nCNOT + info_subcircuit.nCNOT ; 
            end
    ...
    for k = 1:n-1
        para_row_index = pow2(k):pow2(k+1)-1;
        [circuit_GLOBAL, info_subcircuit ] = UniformRotation( circuit_GLOBAL, 'RY', Varphi_global(para_row_index,:)', int32(0:n-1), int32(k), logging, circuit_sim ) ;
        if logging 
            nRY = nRY + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ;
        end
    ...
    while i <= pow2(n_count)
        if any(para_seq(:,i) ~= 0)
            ...
            if size(para_seq, 1) == 1
                ...
                [ circuit, info_subcircuit, parity_check ] = UniformRotation( circuit, ctrl_type, para_seq(:,i)', n + ctrl_index, targ_index, logging, circuit_sim );
                if logging 
