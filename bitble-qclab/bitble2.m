function [circuit, subnormalization_factor, info] = bitble2( A, compr_type, compr_val, logging, offset, circuit_sim )
...
%         info     -- struct containing some info on compression algorithm and circuit
    ...
    if iscomplex
        ...
        [ circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RZ', Theta(end-pow2(n)+1:end)', ctrl_index, 0, logging, circuit_sim ); 
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ;
        end
    end
    [ circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RY', Varphi(1:pow2(n))', ctrl_index, 0, logging, circuit_sim ) ;
    if logging
    ...
    for k = 1:n-1
        ...
        if any( para_seq ~= 0)
            ...
            [ circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RY', para_seq(:)', ctrl_index, k, logging, circuit_sim ) ; 
            if logging
                nRY = nRY + info_subcircuit.nG ;
                nCNOT = nCNOT + info_subcircuit.nCNOT ; 
            end
    ...
    if iscomplex
        [ circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RZ', Theta(1:pow2(n))', n:(2*n-1), 0, logging, circuit_sim ) ;
        if logging
            nRZ = nRZ + info_subcircuit.nG ;
            nCNOT = nCNOT + info_subcircuit.nCNOT ;
        end
        for k = 1:n-1
            ...
            if any( para_seq ~= 0)
                ctrl_index = [0:k-1, n:(2*n-1)] ;
                [ circuit_PREP, info_subcircuit ] = UniformRotation( circuit_PREP, 'RZ', para_seq(:)', ctrl_index, k, logging, circuit_sim ) ; 
                if logging
                    nRZ = nRZ + info_subcircuit.nG ;
                    nCNOT = nCNOT + info_subcircuit.nCNOT ; 
                end
    ...
    for k = 1 : n-1 
        para_row_index = pow2(k):pow2(k+1)-1 ;
        [circuit_GLOBAL, info_subcircuit ] = UniformRotation( circuit_GLOBAL, 'RY', Varphi_global(para_row_index)', setdiff(0:n-1,k), k, logging, circuit_sim ) ; 
        if logging
            nRY = nRY + info_subcircuit.nG ; 
            nCNOT = nCNOT + info_subcircuit.nCNOT ; 
        end
