...
% circuit_sim:  true/false, if true info will simulate the circuit, else
%               only compute the single-qubit rotation angles
%
% OUTPUT
% ------
...
function [ circuit, info ] = UniformRotation2( circuit, ctrl_type, para_seq, ctrl_index, targ_index, logging, circuit_sim )
...
% output: exp(-0.5i.*global_phase).*diag(circuit.matrix).' = exp(theta.*1i)
    ...
    for i = 2:n
        ...
        [circuit, info]  = UniformRotation2(circuit,'RZ',theta(pow2(i-1)+1:pow2(i))',0:i-2,i-1,logging, circuit_sim) ;
        if logging 
...
% -------------------------------------------------------------------------
    ...
    [circuit, info ] = UniformRotation2( circuit,'RY',theta,ctrl_index,theta_index-1, logging, circuit_sim );

end % end of double_ucry
...
function convertedAngles = AngleCompute( ctrl_type, NormOrPhase )
