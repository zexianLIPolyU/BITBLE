...
%% transform data
% Method 1 to generate 'thetat'
thetat = grayPermutation(sfwht(theta));

% Method 2 to generate 'thetat'
%{
% Generate the Walsh-Hadamard matrix times permutation matrix 'M'
N = length(theta);
n = ceil(log(N)/log(2));

% b_{i-1} and g_{i-1} bitwise multipulication 'BinaryTimesGray'
BinaryTimesGray = bitand((0:N-1)',grayCode(0:N-1));
M = zeros(N,N);

% Counting the parity of each element in BinaryTimesGray
for i = 1:n
